...
const REQUIRED: RequiredSpec[] = [
  ...
  },
  {
    ...
    detail: 'Add a provider key under Secrets, then select the provider under Connections.',
    href: '/settings/connections',
...
const OPTIONAL: ReadonlyArray<Omit<SetupItem, 'status'>> = [
  {
    ...
    detail: 'Optional: connect a read-only account-data source so the agent can investigate a customer’s account state.',
    href: '/settings/connections',
    keys: [],
    unmet: [],
  },
  {
    ...
    detail: 'Optional: upload docs (or import from Drive/Dropbox/OneDrive) to ground L1 answers in your product.',
